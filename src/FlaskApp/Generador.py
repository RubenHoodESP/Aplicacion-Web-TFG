...
# Quitamos todas las variantes que tengan huecos o 
# solapamientos
def reparador_final(trazados):

    trazados_finales = [Trazador.trazar_A(), Trazador.trazar_B(), 0]

    for trazado in trazados:
        trazado_aux = copy.deepcopy(trazados_finales)
        trazado_aux[2] = trazado

        z = trazado_aux[0] | trazado_aux[1]
        zz = z | trazado_aux[2]
        zz2 = sorted(zz.items())
